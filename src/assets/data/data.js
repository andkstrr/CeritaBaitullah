export const nav = [
    {
      id: 1,
      text: "home",
      url: "/",
    },
    {
      id: 2,
      text: "about",
      url: "/about",
    },
    {
      id: 3,
      text: "blog",
      url: "/blog",
    },
    {
      id: 3,
      text: "contact",
      url: "/contact",
    },
  ]
  export const category = [
    {
      id: 1,
      category: "tips",
      title: "Stay Calm And Surf",
      cover: "/images/category/ca1.png",
    },
    {
      id: 2,
      category: "Fashion",
      title: "Becoming a Dragonfly",
      cover: "/images/category/ca2.png",
    },
    {
      id: 3,
      category: "Travel",
      title: "There's always light at the end of the tunnel",
      cover: "/images/category/ca3.png",
    },
    {
      id: 4,
      category: "Sport",
      title: "Stay Calm And Surf",
      cover: "/images/category/ca4.png",
    },
    {
      id: 5,
      category: "Fun",
      title: "There's always light at the end of the tunnel",
      cover: "/images/category/ca5.png",
    },
    {
      id: 6,
      category: "Health",
      title: "Becoming a Dragonfly",
      cover: "/images/category/ca6.png",
    },
    {
      id: 7,
      category: "Business",
      title: "Stay Calm And Surf",
      cover: "/images/category/ca7.png",
    },
    {
      id: 8,
      category: "Technology",
      title: "There's always light at the end of the tunnel",
      cover: "/images/category/ca8.png",
    },
  ]
  export const blog = [
    {
      id: 1,
      title: "7 Amalan di Bulan Ramadhan Yang Bisa Menambah Pahala",
      desc: "Bulan Ramadan adalah bulan yang penuh keberkahan dan rahmat dari Allah SWT. Pada bulan ramadhan ini, pintu-pintu surga dibuka, pintu-pintu neraka ditutup, dan setiap amal kebaikan akan dilipatgandakan pahalanya. Rasulullah ṣallallāhu 'alayhi wa sallam bersabda:",
      category: "tips",
      cover: "/images/blogs/b1.jpg",
      author: "Naswa",
      date: "24 Februari 2025",
    },
  
    {
      id: 2,
      title: "Wajib Praktik! 12 Cara Antimager Khatam Al-Qur'an di Bulan Ramadhan",
      desc: "Pernah niat khatam Al-Qur’an di bulan Ramadan, tapi nggak kesampaian? Awalnya semangat, tapi makin ke sini mulai banyak alasan: sibuk, capek, atau malah ketiduran habis sahur. Akhirnya, niat tinggal niat. Padahal, Ramadan adalah waktu terbaik untuk memperbanyak ibadah, termasuk membaca Al-Qur’an. Sayang kalau cuma jadi rencana tanpa usaha yang jelas. Membaca Al-Qur’an bukan cuma soal selesai khatam, tapi juga ibadah yang penuh keberkahan.",
      category: "tips",
      cover: "/images/blogs/b2.jpg",
      author: "Naswa",
      date: "17 Februari 2025",
    },
    {
      id: 3,
      title: "Khutbah Jum'at: Meneladani Kepemimpinan Rasulullah SAW",
      desc: "Jamaah Jumat yang dimuliakan Allah, Pada hari yang penuh keberkahan ini, marilah kita panjatkan puji dan syukur kepada Allah SWT atas segala nikmat yang telah dilimpahkan kepada kita, terutama nikmat iman dan Islam. Tak lupa, shalawat serta salam semoga selalu tercurah kepada junjungan kita Nabi Muhammad SAW, keluarga, sahabat, serta pengikutnya hingga akhir zaman. ",
      category: "khazanah",
      cover: "/images/blogs/b3.jpg",
      author: "Naswa",
      date: "29 November 2024",
    },
    {
      id: 4,
      title: "Kisah Nabi Muhammad SAW dan Mukjizat Keringatnya yang Harum",
      desc: "Nabi Muhammad SAW adalah manusia pilihan Allah yang diberikan berbagai mukjizat, salah satunya adalah mukjizat keringatnya yang memiliki aroma wangi. Berikut adalah kisahnya:",
      category: "kisah",
      cover: "/images/blogs/b4.jpg",
      author: "Naswa",
      date: "27 Juni 2024",
    },
    {
      id: 5,
      title: "Aturan Khusus Wanita Selama di Masjidil Haram dan Nabawi, yang Dikeluarkan Saudi!",
      desc: "Arab Saudi telah mengeluarkan panduan khusus bagi jemaah wanita yang beribadah di Masjidil Haram dan Masjid Nabawi. Pedoman ini diumumkan oleh Otoritas Umum untuk Perawatan Urusan Masjidil Haram dan Masjid Nabawi melalui akun resmi mereka di platform X. Kebijakan tersebut dirancang untuk memastikan kesucian dan kenyamanan kedua masjid suci tersebut.",
      category: "kabar haji umroh",
      cover: "/images/blogs/b5.jpg",
      author: "Naswa",
      date: "12 Desember 2024",
    }
  ]